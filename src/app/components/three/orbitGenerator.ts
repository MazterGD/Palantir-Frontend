...
export class OrbitGenerator {
  ...
  private solveKepler(e: number, M: number): number {
    ...
    // Fallback for high eccentricity: use binary search if Newton's method fails
    if (count >= maxIterations) {
      ...
      for (let i = 0; i < 50; i++) {
        const residual = E0 - e * Math.sin(E0) - MNorm;
  ...
  getCurrentStateVectors(julianDate: number): StateVectors {
    ...
    // FIXED: Correct velocity calculation with proper denominator
    const muSun = 1.32712440018e11;
    const denominator = 1 - e * cosE;
    const sqrtMuOverA = Math.sqrt(muSun / a);

    const vx = (-sqrtMuOverA * sinE) / denominator;
    const vy = (sqrtMuOverA * Math.sqrt(1 - e * e) * cosE) / denominator;
  ...
  static fromStateVectors(
    ...
    // FIXED: Compute true anomaly and mean anomaly properly
    let trueAnomaly = 0;
    if (e !== 0) {
      const cosNu =
        (position.x * ex + position.y * ey + position.z * ez) / (r * e);
      trueAnomaly = Math.acos(Math.max(-1, Math.min(1, cosNu)));
      if (dot < 0) trueAnomaly = 2 * Math.PI - trueAnomaly;
    }

    // Convert true anomaly to eccentric anomaly
    const cosE = (e + Math.cos(trueAnomaly)) / (1 + e * Math.cos(trueAnomaly));
    const eccentricAnomaly = Math.acos(Math.max(-1, Math.min(1, cosE)));
    ...
    // Compute proper perihelion time
    const perihelionTime = epoch - meanAnomaly / meanMotion;
    ...
    return {
      ...
      perihelionTime, // FIXED: Now correctly computed
      meanAnomaly, // FIXED: Now correctly computed
      meanMotion,
      epoch,
    };
  }
